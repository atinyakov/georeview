...
function init () {
    ...
    if (localStorage.data !== undefined) {
        ...
        pins.forEach((pin) => {
            mapEls[idx] = { coords: pin.coords, name: pin.name, point: pin.point, date: pin.date, message: pin.message}
            idx++;

            ymaps.geocode(pin.coords)
            .then(function (res) {
            var address = res.geoObjects.get(0).properties.get('text');
            var d = new Date(Date.parse(pin.date));

            var placemark = new ymaps.Placemark(pin.coords, {
                balloonContentHeader: `<b>${pin.point}</b>`,
                balloonContentBody: `<div id="review"><a class="linkCoords" href="javascript:void(0);" data-coords="${pin.coords}">${address}</a> <p>${pin.message}</p></div>`,
                balloonContentFooter: `${d.getDate()}.${d.getMonth()}.${d.getFullYear()} ${d.getHours()}:${d.getMinutes()}`,
            }, {
                balloonContentLayout: '<div class="form">' + 
                        '<form >' +
    ...
    function createBaloon (coords, object) {
        ...
        var BalloonContentLayout = ymaps.templateLayoutFactory.createClass(
            ...
            addContent: function (coords, name, message, point, d) {
                ...
                if(localStorage.data !== undefined) {
                    idx = localStorage.data.length + 1;
                }

                mapEls[idx] = { coords: coords, name: name, point: point, date: d, message: `<div id="review"><b>${name}</b> <span>${point}</span><span class="data">${d.getDate()}.${d.getMonth()}.${d.getFullYear()} ${d.getHours()}.${d.getMinutes()}</span><p>${message}</p></div>`}
